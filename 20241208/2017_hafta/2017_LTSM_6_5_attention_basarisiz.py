...
from tensorflow.keras.layers import Input, LSTM, Dense, Flatten, RepeatVector, Permute, Multiply, Activation, Lambda
...
# CUSTOM FEATURES
df['geri_donus_fark'] = df['geri_donus_sayisi'].diff().fillna(0)  # momentum gibi çalışır

df['geri_donus_3hafta'] = df['geri_donus_sayisi'].rolling(window=3).mean()
df['geri_donus_3hafta'] = df['geri_donus_3hafta'].fillna(method='bfill')  # eksik değerleri doldur
...
scaled_data = scaler.fit_transform(df[[
    'geri_donus_sayisi', 'rejim_degisti',
...
input_layer = Input(shape=(look_back, X_all.shape[2]))
lstm_out = LSTM(64, return_sequences=True)(input_layer)
context = simple_attention(lstm_out)
...
dummy_fark = np.zeros_like(rejim_all)
dummy_ort = np.zeros_like(rejim_all)
dummy_oran = np.zeros_like(rejim_all)

combined_all = np.hstack((np.array(rolling_predictions).reshape(-1, 1),
                          rejim_all, dummy_fark, dummy_ort, dummy_oran))
...
for _ in range(future_weeks):
    ...
    new_step = np.array([next_pred, 1.0, 0.0, 0.0, 0.0])
...
dummy_fark = np.zeros((future_weeks, 1))
dummy_ort = np.zeros((future_weeks, 1))
dummy_oran = np.zeros((future_weeks, 1))
future_combined = np.hstack((future_scaled, dummy_rejim, dummy_fark, dummy_ort, dummy_oran))
